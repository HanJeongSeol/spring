...
public class TVUser {
	public static void main(String[] args) {
		...
		GenericXmlApplicationContext factory = 
				new GenericXmlApplicationContext("applicationContext_01.xml"); // 스프링 컨테이너를 구동시킬 수 있다.
		System.out.println("------ 스프링 컨테이너 구동후 ------");

		//2. 스프링컨테이너 사용
		System.out.println("--- tv 요청 사용 ---");
		TV tv = (TV)factory.getBean("tv1Samsung");  // tv라는 이름의 객체가 있다면 불러와서 사용한다.
		tv.powerOn();
		tv.volumnUp();
		tv.volumnDown();
		tv.powerOff();
		...
		tv = (TV)factory.getBean("tv1");  // tv라는 이름의 객체가 있다면 불러와서 사용한다.
		tv.powerOn();
		tv.volumnUp();
		tv.volumnDown();
		tv.powerOff();
